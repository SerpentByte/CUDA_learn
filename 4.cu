...
{
	__shared__ xyz sharedPoints[blocksize];
	size_t temp, closest=0;
	...
	for(size_t current_points_block=0;
		current_points_block<gridDim.x; current_points_block++)
	{		
		temp = threadIdx.x + current_points_block*blocksize;
		if(temp<n)
		{
			sharedPoints[threadIdx.x] = points[temp];
		}
		__syncthreads();
		if(id<n)
		{
			double* ptr = &sharedPoints[0].x;
			for(size_t i=0;i<blocksize;i++)
			{
				distance = pow((priv_point.x - ptr[0]),2);
				distance += pow((priv_point.y - ptr[1]),2);
				distance += pow((priv_point.z - ptr[2]),2);
				//distance = sqrt(distance);
				if(distance<d &&
					i+current_points_block*blocksize!=id && 
					i+current_points_block*blocksize<n)
				{
					closest = i+current_points_block*blocksize+1;
					d = distance; 
				}
				ptr +=3;
			}
